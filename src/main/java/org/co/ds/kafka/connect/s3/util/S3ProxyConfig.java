...
package org.co.ds.kafka.connect.s3.util;
...
public class S3ProxyConfig {
  ...
  public S3ProxyConfig(S3SinkConnectorConfig config) {
    try {
      URL url = new URL(config.getString(S3SinkConnectorConfig.S3_PROXY_URL_CONFIG));
      protocol = extractProtocol(url.getProtocol());
      host = url.getHost();
      port = url.getPort();
      String username = config.getString(S3SinkConnectorConfig.S3_PROXY_USER_CONFIG);
      user = StringUtils.isNotBlank(username)
             ? username
             : extractUser(url.getUserInfo());
      Password password = config.getPassword(S3SinkConnectorConfig.S3_PROXY_PASS_CONFIG);
      ...
      throw new ConfigException(
          S3SinkConnectorConfig.S3_PROXY_URL_CONFIG,
          config.getString(S3SinkConnectorConfig.S3_PROXY_URL_CONFIG),
          e.toString()
      );
    }
  }
