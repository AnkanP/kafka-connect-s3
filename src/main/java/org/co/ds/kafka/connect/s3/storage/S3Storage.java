...
package org.co.ds.kafka.connect.s3.storage;
...
import org.co.ds.kafka.connect.s3.util.S3ProxyConfig;
...
import org.co.ds.kafka.connect.s3.format.parquet.ParquetFormat;
...
public class S3Storage implements Storage<S3SinkConnectorConfig, ObjectListing> {
  ...
  public AmazonS3 newS3Client(S3SinkConnectorConfig config) {
    ...
    String region = config.getString(S3SinkConnectorConfig.REGION_CONFIG);
  ...
  public ClientConfiguration newClientConfiguration(S3SinkConnectorConfig config) {
    ...
    if (StringUtils.isNotBlank(config.getString(S3SinkConnectorConfig.S3_PROXY_URL_CONFIG))) {
  ...
  protected RetryPolicy newFullJitterRetryPolicy(S3SinkConnectorConfig config) {
    PredefinedBackoffStrategies.FullJitterBackoffStrategy backoffStrategy =
        new PredefinedBackoffStrategies.FullJitterBackoffStrategy(
            config.getLong(S3SinkConnectorConfig.S3_RETRY_BACKOFF_CONFIG).intValue(),
            S3SinkConnectorConfig.S3_RETRY_MAX_BACKOFF_TIME_MS
        );
