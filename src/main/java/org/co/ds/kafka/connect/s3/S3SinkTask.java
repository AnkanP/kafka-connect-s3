...
import org.co.ds.kafka.connect.s3.util.TombstoneSupportedPartitioner;
import org.co.ds.kafka.connect.s3.util.SchemaPartitioner;
...
import org.co.ds.kafka.connect.s3.format.KeyValueHeaderRecordWriterProvider;
import org.co.ds.kafka.connect.s3.format.RecordViewSetter;
import org.co.ds.kafka.connect.s3.format.RecordViews;
import org.co.ds.kafka.connect.s3.util.Version;
  ...
  private boolean maybeSkipOnNullValue(SinkRecord record) {
    if (record.value() == null) {
      if (connectorConfig.nullValueBehavior()
          .equalsIgnoreCase(S3SinkConnectorConfig.OutputWriteBehavior.IGNORE.toString())) {
        log.debug(
        ...
        return true;
          ...
          .equalsIgnoreCase(S3SinkConnectorConfig.OutputWriteBehavior.WRITE.toString())) {
        log.debug(
